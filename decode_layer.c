...
float *AdaptiveWeights(iftMImage *mimg, float perc_thres)
{
  ...
  for (int b=0; b < mimg->m; b++){ // para cada canal (kernel)

    // printf("n:%ld  ", mimg->n);
    // printf("m:%ld\n", mimg->m);
    // printf("xsize:%ld  ", mimg->xsize);
    // printf("ysize:%ld  ", mimg->ysize);
    // printf("zsize:%ld  ", mimg->zsize);

    float sum = 0;
    for (int p = 0; p < mimg->n; p++){
      // printf("%d %d %f\n", iftMGetVoxelCoord(mimg,p).x, iftMGetVoxelCoord(mimg,p).y, mimg->val[p][b]);
        sum += mimg->val[p][b];
    }

    float mu = sum / mimg->n;

    weight[b] = mu>=perc_thres? -1.0:1.0;

    // printf("\n\n\nmu:%f\n, th:%f, weight[b]:%f", mu, perc_thres, weight[b]);
  ...
  for(int i = 0; i < fs->n; i++) {
    ...
    if (weight != NULL){ 	
      ...
      for (int p=0; p < mimg->n; p++){
        ...
        // salie->val[p] = 1.0 / (1.0 + exp(-salie->val[p]));
      }
      iftFree(weight); 
